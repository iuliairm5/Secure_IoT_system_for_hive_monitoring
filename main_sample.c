...
uint8_t datareadyflagLIS = 0; //un flag care arată că datele încă nu sunt pregătite a fi citite cu 
//LIS3DH
----------------------------------------------------------------------------------------------------

//cod din main
//citirea datelor de la ADC
//deoarece am senzori pe diferite canale ale unui ADC, doresc să comut între ele la un interval
//anume de timp (voi începe mereu cu senzorul de pe canalul 3 deși el este canalul by default 
//selectat de către Cube)
...
if(HAL_ADC_PollForConversion(&hadc2, 5) == HAL_OK) //aștept 5 ms iar dacă această conversie s-a 
//realizat cu succes, abia atunci voi citi valorile cu ADC
{
semnal_temp_ghem_necriptat = HAL_ADC_GetValue(&hadc2); //salvez datele citite în variabila 
//corespunzătoare
}
...
HAL_ADC_Start(&hadc2); 
if(HAL_ADC_PollForConversion(&hadc2, 5) == HAL_OK) 
{vscap_adc = HAL_ADC_GetValue(&hadc2);//citesc valoarea VSCap cu care apoi pot face acea condiție 
//IF
}
----------------------------------------------------------------------------------------------------
//comparatorul care lucrează în mod continuu și trezește MCU dacă sunt zgomote suspecte de la 
//microfon 
dac_ref_byte = (uint8_t)((dac_referinta/3.3)*255); //fac conversie de la floating point (domeniul
//0V-3.3V) la byte (domeniul 0 -255), altfel nu v-a compila tocmai bine
HAL_DAC_Start(&hdac1, DAC_CHANNEL_1);//pornesc DAC-ul pe canalul 1
HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_8B_R, dac_ref_byte); //am la ieșirea DAC tensiunea 
//de referință de 1.25V pe 8 biți
HAL_COMP_Start(&hcomp1); //pornesc comparatorul
if(HAL_COMP_GetOutputLevel(&hcomp1) == COMP_OUTPUT_LEVEL_HIGH) //verific dacă tensiunea citită de la 
//microfon > 1.25V
{ ...... //trezesc microcontroller-ul }

//citire LIS3DH cu I2C (am setat external line interrupt pe pinul PC13 iar apoi am activat 
//întreruperea din NVIC)
...
myconfigLIS.fullScale = LIS3DH_FULLSCALE_4; //domeniul senzorului va fi de +/4g ( g este accelerația 
//gravitațională; 1g=9.8m/s^2)
myconfigLIS.antiAliasingBW = LIS3DH_FILTER_BW_50;//lățimea de bandă anti-aliasing setată la 50Hz 
//pentru a nu se updata prea repede (este suficientă pentru a elimina zgomotele de înaltă frecvență)
...
uint8_t i;
for(i=0;i<255;i++)
{if(HAL_I2C_IsDeviceReady(&hi2c2, i, 1, 10) == HAL_OK)//testez adresa SLAVE; încerc o singură dată 
//cu timeout=10ms 
{ break;}
}
return i; //folosesc această adresă în funcțiile HAL_I2C_Master_Transmit sau Receive
}
if((LIS3DH_PollDRDY(1000) == true) || (datareadyflagLIS == 1)) //aștept 1 sec ca data să fie 
//pregătită a fi citită sau am o întrerupere externă
